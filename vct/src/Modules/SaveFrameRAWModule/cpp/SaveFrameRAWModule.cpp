...
SaveFrameRAWModule::SaveFrameRAWModule()
:m_filename(""),
 m_frame_to_write(-1),
...
void SaveFrameRAWModule::SetParameter(const string & name, const string & value)
{
	...
	else if(sName == "componenttowrite")
	{
		...
		{
			m_pMevicLogger->logError("SaveFrameRAWModule::SetParameter: the parameter provided in \"ComponentToWrite\" cannot be casted to int");
		}
	}
	else if(sName == "frametowrite")
	{
		...
		{
			m_pMevicLogger->logError("SaveFrameRAWModule::SetParameter: the parameter provided in \"FrameToWrite\" cannot be casted to int");
		}
	}
	else if(sName == "channeltowrite")
	{
		...
		{
			m_pMevicLogger->logError("SaveFrameRAWModule::SetParameter: the parameter provided in \"ChannelToWrite\" cannot be casted to int");
		}
	}
	else
	{
		m_pMevicLogger->logError("SaveFrameRAWModule::SetParameter: Invalid parameter name");
	}
}

void SaveFrameRAWModule::Simulate( vector<DSContainer*>& list)
{
	...
	//files write
	if (!container->SaveFrameRAW(m_component_to_write, m_frame_to_write, m_filename.c_str(), m_channel_to_write))
	{
		m_pMevicLogger->logError("SaveFrameRAW call failed");
	}
