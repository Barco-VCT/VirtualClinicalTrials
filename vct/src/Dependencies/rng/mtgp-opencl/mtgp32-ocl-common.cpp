 ...
 */
void make_kernel_data(cl::CommandQueue cmdQueue, cl::Buffer d_status,
			mtgp32_params_fast_t params[],
			int block_num) {
	mtgp32_kernel_status_t* h_status = (mtgp32_kernel_status_t *) malloc(sizeof(mtgp32_kernel_status_t) * block_num);
	if (h_status == NULL) {
...
#endif
	cl_int status = cmdQueue.enqueueWriteBuffer(d_status, CL_TRUE/*blocking*/, 0/*offset*/, sizeof(mtgp32_kernel_status_t) * block_num, h_status, 0, 0);
	CL_CHECK(status, "copy d_status to the device");
	free(h_status);
}
